...
## 12. 서버 액션에서 외부 API 호출 (api 폴더 없이) ★
질문: "use server에서 API 호출 대표 예시 / api 폴더 따로 안 만들고 구현한 부분"
핵심: 서버 액션(또는 서버 전용 함수)에서 fetch로 외부 API를 직접 호출. Route Handler(app/api) 안 만들어도 됨.

실제 코드 — 토스 결제 승인(lib/api/toss/confirm-payment.ts, 서버에서만 실행):
```typescript
const secretKey = process.env.TOSS_WIDGET_SECRET_KEY;  // ★ 서버에서만 접근(NEXT_PUBLIC_ 아님)
...
export async function confirmPayment({ paymentKey, orderId, amount }) {
  ...
  const response = await fetch('https://api.tosspayments.com/v1/payments/confirm', {
    ...
    body: JSON.stringify({ paymentKey, orderId, amount }),  // 금액 재검증용 값
  });
  const data = await response.json();
  if (!response.ok) throw new Error(`Toss confirm failed: ${data.message} (${data.code})`);
  return data;
}
```
★ 왜 이게 중요한가:
- 시크릿 키(TOSS_WIDGET_SECRET_KEY)를 서버에서만 사용 → 브라우저에 절대 노출 안 됨. (환경변수에 NEXT_PUBLIC_ 안 붙이면 서버 전용)
- 클라가 직접 토스 API를 부르지 않고, 서버가 대신 부르며 금액을 재검증 → 위변조 방지("프론트 값 안 믿기").
- api 폴더(Route Handler)를 따로 안 만들어도, 서버 함수/서버 액션에서 fetch로 외부 연동 완성.

서버 액션에서 DB+외부 API+리다이렉트를 한 번에(payments.actions.ts):
```typescript
'use server';
import { prisma } from '@/db/prisma';
import { redirect } from 'next/navigation';
// 결제 완료 저장 → prisma.finishedPayment.create(...)
// 권한 없으면 → redirect('/unauthorized')
```
★ 정리 — 언제 서버 액션 vs Route Handler:
- 서버 액션(fetch 포함): 우리 앱 내부 흐름에서 부르는 것(결제 승인, DB 저장). api 폴더 불필요.
- Route Handler(app/api): 외부가 우리를 부르는 것(Stripe 웹훅 api/webhooks/stripe), 외부에서 접근할 URL이 필요한 것(SMS, NextAuth).
- 이 프로젝트는 토스 confirm은 서버 함수 fetch로, Stripe 웹훅은 Route Handler로 → 방향에 따라 갈랐다.
