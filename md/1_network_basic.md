...
TLS 1.3 순서:
1. Client Hello: 알고리즘·해시함수 목록 제안 + 클라 임시 DH 공개값 전송.
2. Server Hello: 알고리즘 하나 선택 + 서버 임시 DH 공개값 전송.
3. 서버 신원 자료 전송:
...
5. 양쪽이 비밀키 계산: 클라(내 임시 개인키+서버 임시 공개값), 서버(내 임시 개인키+클라 임시 공개값) → 수학적으로 동일한 대칭키 완성. 여기까지가 핸드셰이크.
6. 실제 데이터 통신: 이후 모든 데이터는 이 대칭키 하나로 암복호화+무결성(AES-GCM 태그). 공개키/개인키는 서랍에 넣고 안 씀.
...
한 줄: 룰·DH 공개값 교환(1,2) → 서버 진위 인증(3,4) → 각자 같은 비밀키 계산(5) → 그 대칭키로 실제 데이터(6). 비대칭=인증(앞), 대칭=데이터(뒤).
